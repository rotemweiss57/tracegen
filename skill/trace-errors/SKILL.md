...
Pipe the failed command's output through TraceGen:

```bash
<failed-command> 2>&1 | npx tracegen analyze-output --condensed --no-search --no-git
```

Or re-run the failing command through TraceGen directly:

```bash
npx tracegen analyze --command '<failed-command>' --condensed --no-search --no-git
```
